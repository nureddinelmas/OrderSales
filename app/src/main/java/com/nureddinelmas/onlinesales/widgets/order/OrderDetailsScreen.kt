	...
	Column {
		...
		if (!showProductsDialog) {
			...
			Row(
				...
				Text(
					text = if(currentOrder.productList.isNotEmpty()) "Shipping: ${currentOrder.shipping.toSekFormat(currentOrder.productList[0].productCurrency.uppercase())}" else "Shipping: 0 SEK",
					style = TextStyle(fontSize = 18.sp, fontWeight = FontWeight.Bold),
					textAlign = TextAlign.Center
				)
			}
			Row(
				...
				Text(
					text = if(currentOrder.productList.isNotEmpty()) "Total Price: ${currentOrder.totalPrice().toSekFormat(currentOrder.productList[0].productCurrency.uppercase())}" else "Total Price : 0 SEK",
					style = TextStyle(fontSize = 18.sp, fontWeight = FontWeight.Bold),
					textAlign = TextAlign.Center
				)
			}
